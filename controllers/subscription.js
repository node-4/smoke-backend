...
exports.getMonthlySubscription = async (req, res) => {
  try {
    ...
    if (findSubscription) {
      ...
      return res.status(404).json({ status: 404, message: "Subscription not found.", data: {} });
    }
  } catch (err) {
    return res.status(500).json({ message: err.message });
  }
...
exports.takeSubscription = async (req, res) => {
  try {
    ...
    if (!findSubscription) {
      return res.json({ status: 404, message: 'subscription not found.', data: {} });
    } else {
      let obj = {
        userId: req.user.id,
  ...
  try {
    ...
    if (findSubscription) {
      if (findSubscription.name == "Monthly") {
        ...
        return res.json({ status: 200, message: 'subscription subscribe successfully.', data: update });
      }
...
exports.useSubscriptionvalue = async (req, res) => {
  try {
    ...
    if (findSubscription) {
      ...
      if (update) {
        return res.json({ status: 200, message: 'subscription first name successfully.', data: update });
      }
    } else {
      return res.json({ status: 404, message: 'subscription not found.', data: {} });
    }
...
exports.updatevalue = async (req, res) => {
  try {
    ...
    if (findSubscription) {
      let update = await userSubscription.findByIdAndUpdate({ _id: findSubscription._id }, { $set: { fullName: findSubscription.fullName + 1 } }, { new: true });
      if (update) {
        return res.json({ status: 200, message: 'subscription first name count increasse successfully.', data: update });
      }
    } else {
      return res.json({ status: 404, message: 'subscription not found.', data: {} });
    }
